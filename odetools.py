...
def run_simulation(*, resume=False, n_runs, n_nodes, a, b, inner_probs, outer_probs):
    ...
    if not resume:
        ...
        with sqlc.SQLConnector() as dbconn:
            ...
            dbconn.cursor.execute("INSERT INTO simulations VALUES (%s,%s,%s,%s,%s,%s,NULL)" % (sim_id, n_runs, n_nodes, 2,
                                                                                        a, b))
            dbconn.cursor.execute("INSERT INTO communities VALUES ({0},1,{1}),({0},2,{1})".format(sim_id, n_nodes//2))
    ...
    with sqlc.SQLConnector() as dbconn:
        ...
        dbconn.cursor.execute("SELECT run_id,p_inner,p_outer FROM runs WHERE sim_id = %s AND done IS NULL", (sim_id,))
    ...
    for run_id, p_in, p_out in run_params:
        ...
        with sqlc.SQLConnector() as dbconn:
            ...
            dbconn.cursor.execute("UPDATE runs SET done = CURRENT_TIMESTAMP() WHERE run_id = %s", (run_id,))
            dbconn.commit()
